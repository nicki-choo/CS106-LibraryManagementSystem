    ...
    ui(new Ui::Prebook)
{
    ...
    //make connection
    connect(ui->tableView, &QTableView::doubleClicked, this, &Prebook::showInfo);
}
...
void Prebook::on_SearchBookBtn_clicked()
{
    //Get the ID
...
void Prebook::on_SearchMemberBtn_clicked()
{
    //Get the ID
...
void Prebook::on_PREBOOK_clicked()
{
    //call the main Database
    digitalLibrary lib;
    auto db = lib.db;
    ...
    if(Datetime_1 > Datetime_2)
        ...
        auto query = QSqlQuery(db);
        QString insert{"INSERT INTO prebook_list (Book, Member, PrebookFrom, PrebookTo, Note) VALUES ('%1', '%2','%3', '%4', '%5')"};
        if(!query.exec(insert.arg(bookID.toInt()).arg(memberID.toInt()).arg(prebookfrom).arg(prebookto).arg(notes)))
        {
...
void Prebook::showInfo()
{
    ...
    if(!selectedRow.isEmpty())
    {
        ...
        ui->BookIDline->setText(bookID);
        ui->MemberIDLine->setText(memberID);
...
void Prebook::on_DELETE_clicked()
{
    //call the main Database
    digitalLibrary lib;
    auto db = lib.db;
    ...
    QString deleteprebook{"DELETE FROM prebook_list WHERE (Book = '%1') AND (Member = '%2')"};
...
void Prebook::on_CANCEL_clicked()
{
    this->hide();
}
